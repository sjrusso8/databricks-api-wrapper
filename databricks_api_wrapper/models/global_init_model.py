...
#   filename:  global-init-scripts-2.0-azure.json
#   timestamp: 2022-05-14T13:20:03+00:00
...
class ScriptEnabledWithDefault(BaseModel):
    __root__: bool = Field(
        ...,
        description='Specifies whether the script is enabled. The script runs only if enabled.',
        example=False,
...
class ScriptPositionCreate(BaseModel):
...
class ScriptPositionOutput(BaseModel):
    __root__: conint(ge=0) = Field(
        ...,
...
class GlobalInitScriptCreateRequest(BaseModel):
...
class GlobalInitScriptUpdateRequest(BaseModel):
    enabled: Optional[ScriptEnabled] = None
    name: Optional[ScriptName] = None
